...
class DijkstraExplore(Node):
    def __init__(self):
        super().__init__('dijkstra_explore_planner')
    ...
    def current_goal_callback(self, goal_msg):
        # We have reached a goal and we want to plan the next path
        # for the next goal
        current_x = numpy.round(goal_msg.x, 2)
        current_y = numpy.round(goal_msg.y, 2)
        ...
        for node, position in self.positions.items():
            ...
            position_xy = numpy.array([
                position[0], position[1]
            ])

            diff_dist = numpy.round(
                numpy.linalg.norm(current_position - position_xy),
                2
            )
            print(f'Distance difference: {diff_dist}')
