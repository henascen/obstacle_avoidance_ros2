...
class movingTurtle(Node):
    def __init__(self):
        ...
        self.next_pos_sub = self.create_subscription(
            Point32,
            'next_pos_move',
    ...
    def odom_callback(self, odom):
        ...
        self.robot_trajectory.append(self.robot_pos_xy)
    ...
    def angular_vel(self, error_ang_pnt, constant=0.15):
    ...
    def moving_to_point(self):
        ...
        if self.current_goal < len(self.point_list):
            ...
            error_angle = numpy.round(numpy.abs(error_angle_dir), 4)
            ...
            elif distance >= self.tolerance - 0.05:
            ...
            print(f'Final robot pos: {self.robot_pos_xy}')

            plot_trajectory = numpy.array(self.robot_trajectory)
            fig1, ax1 = matpyplot.subplots(1)

            ax1.set_title('Trajectory with Obstacles')
            ax1.scatter(plot_trajectory[:, 0], plot_trajectory[:, 1])

            fig1.tight_layout()
            fig1.savefig('./labs/lab_4/images/t4_trajectory_obstacles.png')
    ...
    def move_to_next_pos(self, next_point_msg):
        next_x = numpy.round(next_point_msg.x, 2)
        next_y = numpy.round(next_point_msg.y, 2)
